...
if [[ $SITE_ID == "" ]]; then
  ...
  RESULT=$(curl -H "Content-Type: application/zip" -H "Authorization: Bearer $NETLIFY_KEY" --data-binary "@pack/$PACKAGE_FILE.zip" $NETLIFY_API/sites)
  ...
  git push -f $GIT_ORIGIN_URL HEAD:$CIRCLE_BRANCH
  ...
  RESULT=$(curl -H "Content-Type: application/zip" -H "Authorization: Bearer $NETLIFY_KEY" --data-binary "@pack/$PACKAGE_FILE.zip" $NETLIFY_API/sites/$SITE_ID/deploys)
